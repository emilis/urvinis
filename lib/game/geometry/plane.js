...
function reduce( plane, fn, acc ){
    ...
    return plane.reduce( reduceRow, acc );

    function reduceRow( acc, row, y ){

        return row.reduce( reducePoint, acc );

        function reducePoint( acc, point, x ){

            return fn( acc, point, x, y );
        }///
    }///
}///
...
function filter( plane, fn ){
    ...
    return reduce( plane, addMatching, [] );

    function addMatching( arr, item, x, y ){

        fn( item, x, y ) && arr.push( item );
        return arr;
    }///
}///
