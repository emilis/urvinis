...
function follow( player, world ){
    ...
    player.on( world.events.MOVE, checkViewport.bind( {}, world ));

    world.events.on( world.events.MOVE, onPlayerMove );
    world.events.on( world.events.GROUND_ADD, onAddGround.bind( {}, world ));

    checkViewport( world, player );
    onPlayerMove( player );
}///
...
function checkViewport( world, player ){
...
function render( world ){
    ...
    function updateBox( worldPlane ){
        ...
        if( moved_viewport ){
            ...
            /// reset stationary layer:
            _.each( stationary, detachBox );
            stationary =        {};
...
function onPlayerMove( player ){
    ...
    var contains =          viewport.contains( player );
    
    if( !contains ){
        if( playerBox && !playerBox.hidden ){
            playerBox.hide();
            renderScreen();
        }
    } else {
        if( !playerBox ){
            movable[player.id] = playerBox = getPlayerBox( player );
            box.append( playerBox );
            playerBox.setIndex( ZINDEX.characters );
        }
...
function onAddGround( world, groundObj ){

    var id =            groundObj.x + ":" + groundObj.y;
    var groundBox =     stationary[id];
    var contains =      viewport.contains( groundObj );

    if( !contains ){
        if( groundBox && !groundBox.hidden ){
            detachBox( groundBox );
            renderScreen();
        }
    } else {
        if( !groundBox ){
            stationary[id] =
                groundBox =
                getGroundBox( groundObj, world.terrain.getAt( groundObj ));
            box.append( groundBox );
            groundBox.setIndex( ZINDEX.ground );
        } else {
            groundBox.setContent(
                getGroundString(
                    groundObj, world.terrain.getAt( groundObj )
                )
            );
        }
        renderScreen();
...
function getGroundBox( groundObj, terrain ){

    return blessed.box({
        left:       2 * ( groundObj.x - viewport.x0 ),
        top:        groundObj.y - viewport.y0,
        width:      2,
        height:     1,
        tags:       true,
        content:    getGroundString( groundObj, terrain ),
    });
}///

function getGroundString( ground, terrain ){
    ...
    switch( ground.type ){
        ...
        case "tracks":
            return getStyledString( ";;",  TERRAIN[ terrain ]);
            break;
        
        default:
            return TERRAIN_STR[ terrain ];
    }
}///
...
function getPixelString( block ){
    ...
    if( block.npc ){
        ...
        return getGroundString( block.ground, block.terrain );  
    } else {
